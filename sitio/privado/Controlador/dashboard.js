...
var dashBoard = function(){
	...
	var divTab  = '';
		...
		divTab += '<div class="w100 boxDash" data-packery=\'{ "itemSelector": ".grid-item", "gutter": 10 }\'>'+cargandoDashBoard+'</div>';
	  ...
	  var funcionLoad = function(query, callback){
	    ...
	    if(_.size(query)>=2){
	      ...
	      var procesaContactos = function(Op, err, adc){ 
	        if(Op){
	        	if (Op.jsonDatos[0].idComponente) {
	...
	this.atrasDetalleIndividualEmpresarial = function(Op){
		...
		if(empresarial){
			$graficaGeneral.slideDown(400, function(){SalesUp.Variables.Reordena()});
	...
	this.verDetalleIndividualEmpresarial = function(Op){
		...
		$boxDetalles.html(compilado).slideDown(400, function(){SalesUp.Variables.Reordena();});
		animacionBarra($p);
	...
	this.nuevoIndicador = function(){
		...
		setTimeout(function(){
			...
			// $('#ltFuentes').focus();
			$('#frmDashIndicador .selectize-input input').focus()
	...
	this.guardaNuevoIndicador = function(Op){
		...
		setTimeout(function(){SalesUp.Variables.Reordena()},500);
...
SalesUp.Variables.CargaFiltrosSistema = function(Op){
	...
	$('#Load'+Paso).hide();
	$OpcionesFiltros.slideDown(400, function(){SalesUp.Variables.Reordena();});
...
SalesUp.Variables.Reordena = function(){
	var TkActual = SalesUp.Variables.tkdbActual;
	var $c = $('#Tabs #dash-'+TkActual+' .boxDash');
	$c.masonry();
	$c.masonry( 'destroy' );
	$c.masonry({
		itemSelector: '.boxPanel'
	});
}

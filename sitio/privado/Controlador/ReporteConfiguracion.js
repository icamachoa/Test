...
SalesUp.Variables.AgrupacionYperiodicidad = function(Op){
	...
	var jsonAgrupacion = SalesUp.Sistema.CargaDatos({Link:'/privado/Modelo/jsonAgrupacionesReportes.dbsp', DataType:'json'}).jsonDatos;
...
SalesUp.Variables.AgregarPaso = function(_Paso){
	...
	var TemplateAgregarPaso = SalesUp.Sistema.CargaDatos({Link:'/privado/Vista/TemplateAgregarPaso.dbsp', Almacen:'TemplateAgregarPaso'});
	var Dato={};
	Dato.Paso = Paso;
	...
	if(Paso > 1){
		$('#NombrePaso'+Paso).attr('style','width:74% !important');
	}
...
SalesUp.Variables.GuardaConfiguracion = function(){
	...
	setTimeout(function() {
		...
		var Configuracion = 'r='+IdReporte+'&t='+todos+'&n='+Nombre+'&a='+Agrupacion+'&p='+Periodicidad+'&g='+CompartirCon;

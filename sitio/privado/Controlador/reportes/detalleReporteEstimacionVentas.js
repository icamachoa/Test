...
SalesUp.Variables.CargaReporteDetalle = function(){
	var Parametros 			= 'start='+Start+'&howmany='+RegXPag+'&filtro='+encodeURIComponent(SalesUp.Variables.FiltrosREV) +'&periodo='+SalesUp.Variables.Periodo+'&parametros='+SalesUp.Variables.Parametros;
	...
	var jsonDatos			= SalesUp.Sistema.CargaDatos({Link:'/privado/Modelo/jsonReporteEstimacionVentasDetalle.dbsp', Parametros:Parametros,DataType:'json'}).jsonDatos;
	if(!_.isUndefined(jsonDatos[0].TOTAL)){
		var total = jsonDatos[0].TOTAL;
	}else{	
		var total = 0;
	}
	var titulo='<input type="hidden" name="titulo" id="titulo" value="Reporte_estimacion_de_ventas_detalle" />';
	...
	SalesUp.exporta.btnExportar({titulo:'Reporte Estimacion'})

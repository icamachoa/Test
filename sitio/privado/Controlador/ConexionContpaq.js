...
SalesUp.Variables.IniciaConfiguracion= function(){
	...
	var jsonActiva = SalesUp.Sistema.CargaDatos({Link:'/privado/Modelo/jsonObtieneActivaEmpresa.dbsp', DataType:'json'}).jsonDatos;
...
SalesUp.Variables.ConstruyeTabsConexion = function() {
	  ...
	  var periodos 			= ["Token", "Configuración"];
	  var Valoresperiodos 	= ["1","2"];
...
SalesUp.Variables.ConstruyeHTML=function(){
	...
	var TemplateDatosToken = SalesUp.Sistema.CargaDatos({Link:'/privado/Vista/TemplateToken.dbsp', Parametros:'thead=1', /*Almacen: 'htmlTemplateToken'*/});
	$('#divTab-1').html(TemplateDatosToken);

	var TemplateDatosConfiguracion = SalesUp.Sistema.CargaDatos({Link:'/privado/Vista/TemplateToken.dbsp', Parametros:'thead=2' /*, Almacen: 'htmlTemplateConfiguracion'*/});
	var TemplateDatos = SalesUp.Sistema.CargaDatos({Link:'/privado/Vista/TemplateToken.dbsp', Parametros:'thead=3'/*, Almacen: 'htmlTemplateTR'*/});
...
SalesUp.Variables.ActivarIntegracion=function(){
	...
	var jsonActivaEmpresa = SalesUp.Sistema.CargaDatos({Link:'/privado/Modelo/jsonActivaComercial.dbsp', DataType:'json'}).jsonDatos;

	var tokenValido = jsonActivaEmpresa[0].TOKEN;
	var ultimaSincro = jsonActivaEmpresa[0].ULTIMA_SINCRONIZACION;
	SalesUp.Variables.Configuracion_local = JSON.parse(jsonActivaEmpresa[0].CONFIG_LOC+'');
	SalesUp.Variables.Sincronizacion_tablas = JSON.parse('['+jsonActivaEmpresa[0].SINC_TAB+']');
	SalesUp.Variables.configuracion = JSON.parse(jsonActivaEmpresa[0].CONFIG);
...
SalesUp.Variables.RegenerarToken=function(){
	...
	var nuevoToken = SalesUp.Sistema.CargaDatos({Link:'/privado/Modelo/jsonRegeneraToken.dbsp', DataType:'json'}).jsonDatos;
	var tokenValido = nuevoToken[0].TOKEN;


	setTimeout(function(){
		$('#token').val(tokenValido);
...
SalesUp.Variables.PopDesactivaEmpresa = function(){
	    ...
	    var Pregunta = 'Esta seguro de desactivar la empresa para integraciones?';
...
SalesUp.Variables.EliminaRegistroIntegracion=function(Op){
	...
	SalesUp.Sistema.CargaDatos({Link:'/privado/Modelo/qryEliminaRegistroEmpresa.dbsp', DataType:'json'});
...
//var $c = $('#popDesactiva');
  ...
  //if(SalesUp.Valida.ValidaObligatorios({DentroDe:$c, DestinoMsj:$('#PopDescuento .BounceOpenInDown ')})){
  	//}

	var $t = $(Op.t);
	var titulo	= $t.attr('data-titulo');
	
	/*identifica desde donde es llamada la pantalla para otener valor de json
	1 lineas de producto
	2 marcas
	*/
	var valorEntero = 0;
	var deDonde = parseInt($t.attr('data-tipo'));

	if(deDonde == 1)	
		valorEntero = parseInt(SalesUp.Variables.Configuracion_local.LINEAS);
		else
			valorEntero = parseInt(SalesUp.Variables.Configuracion_local.MARCAS);
	...
	var Html =	' <div style="margin-top:5px; text-align: left" class="w100 TitDiv">'
			    ...
			    +'			<span class="DescuentoCotizador sFondoMenu BoxSizing" style="font-style: italic !important; font-weight:normal !important">Clasificación en COMERCIAL</span>'
				+'		<div class="BoxInfo w100">'
				+'			<label class="BoxSizing InfoLabel Tip4 Pointer" tip="Clasificación">Numero</label>'
				+'			<input id="clasificacion"  data-tipo="'+deDonde+'" placeholder="Coloca el dato" value="'+valorEntero+'" onKeyPress="return SalesUp.Valida.valDecimales({e:event, t:this, v:value});" class="BoxSizing InfoData InfoObligatorio" type="text" name="clasificacion">'
				+'		</div>'
				+'		<div class="clear"></div>'
				+'	</form> '  
			  	+'</div>';
...
SalesUp.Variables.ConfigurarEmpresa= function(Op){
	...
	var TemplateDatosConfiguracion = SalesUp.Sistema.CargaDatos({Link:'/privado/Vista/TemplateToken.dbsp', Parametros:'thead=6' /*, Almacen: 'htmlTemplateConfiguracion'*/});
	var TemplateDatos = SalesUp.Sistema.CargaDatos({Link:'/privado/Vista/TemplateToken.dbsp', Parametros:'thead=7'/*, Almacen: 'htmlTemplateTR'*/});

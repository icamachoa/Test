...
/******************************************************************************
  ...
  onKeyPress=ValidateFloatCharacter (event, this, car) 
  
******************************************************************************/

		function ValidateFloatCharacter(evento, control,car) {

			var Tecla;
			var valido=false;  var CodVar;
			var esta = Explorador.indexOf("Explorer",0) ;
			if (esta > 0) {
...
/******************************************************************************
  funcion FormatFloatCharacter
	Esta funcion remplaza un caracter por punto.... para formatear moneda
    Se activa cuando vas a hacer un submit mediante jquery
  FormatFloatCharacter (valor, car) 
  
  	   $(document).ready(function(){
			$("#btnAceptar").click(function(){
				var Validar = $("#control").val(); 
				FormatFloatCharacter(Validar,',' )
			});				
	     }); 
  
******************************************************************************/
		function FormatFloatCharacter(valor,car) {
		 var oldcadena; var newcadena;
			 oldcadena=valor;
			 newcadena=oldcadena.replace(car,'.');
		 return newcadena;
		}
	
/*******************/
		function FormatFloatCharacterGen(valor,car,car2) {
		 var newcadena;
			 newcadena=valor.replace(car2,car);
		 return newcadena;
		}
...
//	  res = confirm("¨Esta seguro de querer proceder con la compra?");
  ...
  function ValidaDatosFacturacionCompra(forma) {
    var B_ArrSupport     = new Array ("TELEFONO","CORREO");
    var B_ArrFACT     = new Array ("NOMBRE","RFC", "CALLE","COLONIA", "MUNICIPIO", "CIUDAD", "CODIGOPOSTAL");
    var res = false;
    if ( 
	     (ValidaControlesNoNulos(forma,B_ArrSupport))&&
		 (ValidaCorreo (forma.CORREO.value))
...
function ValidateEmailNow(control){
  ...
  if(finalX>0){
    if((EsLetra(correo.charAt(0)))){ 
       ...
       while((i<finalX)&&(token!='@')&&((EsLetra(token))||(EsNumero(token))||(token=='.')||(token=='_')||(token=='-'))){
          i++;   
       ...
       if ((token == '@')&&(correo.charAt(i+1)!= '.')&&((i+1)<finalX)){
          i++;   
          token = correo.charAt(i)
          while((i<finalX)&&(token!='.')&&((EsLetra(token))||(EsNumero(token))||(token=='_')||(token=='-'))){
              i++;   
              token = correo.charAt(i)
          }
          if((token == '.')&&(correo.charAt(i+1)!='.')&&((i+1)<finalX)){
            ...
            if (i>=finalX){
               ...
               if ((token == '.')&&((i+1)<finalX)){
                 i++;   
                 token = correo.charAt(i)
                 while((i<finalX)&&((EsLetra(token))||(EsNumero(token))||(token=='_')||(token=='-'))){
                   i++;   
                   token = correo.charAt(i)
                 }
                 if(i>=finalX){
...
function ValidaCorreo(correo){
  ...
  if(final>0){
    if((EsLetra(correo.charAt(0)))){ 
       var token = correo.charAt(i)
       while((i<final)&&(token!='@')&&((EsLetra(token))||(EsNumero(token))||(token=='-')||(token=='.')||(token=='_')||(token=='-'))){
          i++;   
          token = correo.charAt(i);
       }
  ...
  function EstandarizaNumero (ElControl) {
    var Num1 = ElControl.value;	
	var Num2 = Num1.replace(/\,/g,'');
    var Num2 = Num2.replace(/\$/g,'');	
	ElControl.value=Num2;
  }

 ...
 */
(function() {
	var n = Highcharts, A = n.Chart, B = n.addEvent, E = n.removeEvent, t = n.createElement, w = n.discardElement, u = n.css, x = n.merge, q = n.each, r = n.extend, F = Math.max, s = document, G = window, C = s.documentElement.ontouchstart !== undefined, y = n.getOptions();
	r(y.lang, {
	...
	r(A.prototype, {
		getSVG : function(b) {
			...
			q(c.series, function(i) {
				...
				q(i.data, function(g) {
					...
					typeof k === "object" && g.config && k.constructor !== Array && r(e, k);
					e.visible = g.visible;
					d.data.push(e);
					if(!h.exporting.enableImages)
						( j = g.config && g.config.marker) && /^url\(/.test(j.symbol) &&
						delete j.symbol
				});
				h.series.push(d)
			});
			a = new Highcharts.Chart(h);
			q(["xAxis", "yAxis"], function(i) {q(c[i], function(g, o) {
					var l = a[i][o], m = g.getExtremes(), p = m.userMin;
					m = m.userMax;
					if(p !==
						void 0 || m !==
						void 0)
						l.setExtremes(p, m, true, false)
				})
			});
		...
		exportChart : function(b, c) {
			...
			q(["filename", "type", "width", "svg"], function(d) {t("input", {
					type : "hidden",
					name : d,
					value : {filename:b.filename||"chart",type:b.type,width:b.width,svg:f}[d]
				}, null, a)
			});
			a.submit();
			w(a)
		...
		contextMenu : function(b, c, a, f, d, k) {
			var e = this, j = e.options.navigation, h = j.menuItemStyle, i = e.chartWidth, g = e.chartHeight, o = "cache-" + b, l = e[o], m = F(d, k), p, z;
			if(!l) {
				e[o] = l = t("div", {
				...
				q(c, function(v) {
					if(v) {
						var D = t("div", {
							...
							onmouseout : function() {u(this, h)
							},
							innerHTML : v.text || e.options.lang[v.textKey]
						}, r({
							cursor : "pointer"
						}, h), p);
						D[ C ? "ontouchstart" : "onclick"] = function() {z();
		...
		addButton : function(b) {
			...
			var a = this, f = a.renderer, d = x(a.options.navigation.buttonOptions, b), k = d.onclick, e = d.menuItems, j = d.width, h = d.height, i, g, o;
			b = d.borderWidth;
			var l = {
			...
			if(d.enabled !== false) {
				i = f.rect(0, 0, j, h, d.borderRadius, b).align(d, true).attr(r({
				...
				g = f.symbol(d.symbol, d.symbolX, d.symbolY, (d.symbolSize || 12) / 2).align(d, true).attr(r(m, {

...
var Minificar = function(){
  var literalStrings;

  this.Minifica = function(Op){
    if(!Op){ console.warn('Se necesitan parametros'); return false; }
    if(!_.isObject(Op)){ console.warn('No es un objeto lo que envian'); return false; }
    return Op.Dato;
    var input, output;
    var i;
    ...
    input = Op.Dato;
    output = input;
    var TamanioInicial = input.length;
    ...
    var TamanioFinal = output.length;

    var Diferencia = TamanioInicial - TamanioFinal;
    if (TamanioInicial > 0)
      var Porcentaje = 100 - Math.round(TamanioFinal / TamanioInicial * 10000) / 100;

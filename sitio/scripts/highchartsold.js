...
(function() {
	...
	function Nd(a, b) {
		function c(p) {
			...
			function x(k, n, t, r) {
				...
				this.alignOptions = {
					...
					verticalAlign : k.verticalAlign || ( va ? "middle" : n ? "bottom" : "top"),
					y : A(k.y, va ? 4 : n ? 14 : -6),
					x : A(k.x, va ? n ? -6 : 6 : 0)
				};
			...
			function V() {
				...
				u(Ha, function(r) {
					...
					if(t) {
						...
						if(r.isCartesian) {u(r.data, function(F) {
			...
			function eb() {
				...
				if(E || Ub) {
					...
					u(Ba, function($, F) {
						if(!Ub || $ >= ia && $ <= ra) {Q && mb[$].isNew && mb[$].render(F, true);
							mb[$].isActive = true;
							mb[$].render(F)
						}
					});
			...
			sa(I, {
				...
				getOffset : function() {
					...
					if(r && r.text) {
						if(!ba) {
							ba = I.axisTitle = ga.text(r.text, 0, 0, r.useHTML).attr({
								zIndex : 7,
		...
		function d() {
			...
			return {
				...
				remove : function(q) {pc(p[q].element);
					p[q] = null
				}
			}
		}
		...
		function e(p) {
			...
			function w(B, D) {
				...
				s = xa ? D : (s + D) / 2;
				o.translate(L, s);
				rd = bb(B - L) > 1 || bb(D - s) > 1 ? function() {w(B, D)
				} : null
			}
			...
			return {
				...
				hide : x,
				destroy : function() {u(da, function(B) {
						B && B.destroy()
					});
					u([I, ba, o], function(B) {
						B && B.destroy()
						...
						if(ud && s.type !== "touchstart") {
							...
							if(Ma > 10) {
								if(sc && (ab || xa) && gc(ta - X, da - ea))
									za || ( za = ga.rect(X, ea, Oa ? 1 : Da, R ? 1 : Aa, 0).attr({
							...
							if(hc && p.shared) {
								...
								for( D = B.length; D--; )B[D]._dist > la && B.splice(D, 1);
		...
		function h() {z.alignTicks !== false && u(Va, function(p) {
				p.adjustTickAmount()
			});
			Ob = null
		}
		...
		function P() {
			function p() {
				...
				if(w && x) {
					if(w !== dd || x !== Jc) {clearTimeout(q);
						q = setTimeout(function() {xd(w, x, false)
						}, 100)
					}
					dd = w;
					Jc = x
		...
		function T() {Pa(l, "endResize", null, function() {
				cd -= 1
			})
		}
		...
		var y = A(z.marginTop, M[0]), C = A(z.marginRight, M[1]), Z = A(z.marginBottom, M[2]), Sa = A(z.marginLeft, M[3]), Na = z.spacingTop, Ea = z.spacingRight, gb = z.spacingBottom, Lb = z.spacingLeft, Rb, uc, Ic, ea, Ib, vb, X, Xb, qb, Zb, oa, Wd, dd, Jc, $a, cb, qd, ad, wc, Kc, Ad, xc, l = this, ke = ( M = z.events) && !!M.click, Bd, gc, hc, ud, jc, Zd, Cd, Aa, Da, Yb, Ec, bd, Fc, $b, ac, Jb, sc = z.showAxes, cd = 0, Va = [], Ob, Ha = [], va, ga, rd, Vd, sd, vd, Gc, yd, zd, xd, td, $d, le = function() {
			...
			function x(G) {
				...
				if(!pa) {
					...
					if(wa && wa.showCheckbox) {
						...
						Qa(G.checkbox, "click", function(Gb) {Pa(G, "checkboxClick", {
			...
			function V() {
				...
				if(ca || la) {
					...
					ac += 2 * R;
					if(D) {
						if($b > 0 && ac > 0) {D[D.isNew?"attr":"animate"](D.crisp(null, null, null, $b, ac));
							D.isNew = false
						}
					} else {
		...
		l.hideLoading = function() {gd(jc, {
				opacity : 0
			}, {
				duration : a.loading.hideDuration,
				complete : function() {Ja(jc, {
						display : jb
					})
				}
			});
			...
			u(c, function(Y, H) {
				...
				u(U, function(y) {
					if(y !== "" || U.length === 1) {
						...
						if(f.test(y)) {Ga(Z, "onclick", 'location.href="' + y.match(f)[1] + '"');
							Ja(Z, {
								cursor : "pointer"
							})
						}
						...
						if(z)
							...
							C.x = g;
						if(!z) {
							if(H) {!Lc && a.renderer.forExport && Ja(Z, {
									display : "block"
								});
		...
		symbol : function(a, b, c, d, e) {
			...
			if(g) {
				...
				if(a)
					...
					hb("img", {
						onload : function() {j(f, be[i] = [this.width, this.height])
						},
						src : i
					})
				}
		...
		ma.prototype = Ca(ed.prototype, {
			...
			clipRect : function(a, b, c, d) {
				...
				return sa(e, {
					...
					updateClipping : function() {u(e.members, function(f) {
	...
	Oc.prototype = {
		...
		applyOptions : function(a) {
			...
			if(lc(a) || a === null)
				this.y = a;
			else if(Nb(a) && !lc(a.length)) {sa(this, a);
				this.options = a
			} else if(Sb(a[0])) {
				this.name = a[0];
				this.y = a[1]
		...
		remove : function(a, b) {
			...
			oc(b, e);
			a = A(a, true);
			c.firePointEvent("remove", null, function() {nc(f, c);
				c.destroy();
				d.isDirty = true;
				a && e.redraw()
			})
	...
	sb.prototype = {
		...
		cleanData : function() {
			...
			if(this.options.connectNulls)
				for( g = b.length - 1; g >= 0; g--)b[g].y === null && b[g - 1] && b[g + 1] && b.splice(g, 1);
		...
		onMouseOver : function() {
			...
			if(!(!Kb && a.mouseIsDown)) {b && b !== this && b.onMouseOut();
			...
			if(this.options.dataLabels.enabled) {
				...
				if(P)
					if(v) {
						...
						if(Y)
							e = Ca(e, {
								y : {top:14,middle:4,bottom:-6}[e.verticalAlign]
							})
					} else if(T)
						e = Ca(e, {
							verticalAlign : "top"
		...
		drawTracker : function() {
			...
			if(d)
				for( j = d + 1; j--; ) {c[j] === Za && c.splice(j + 1, 0, c[j + 1] - f, c[j + 2], Ka);
					if(j && c[j] === Za || j === d)
			...
			if(g)
				...
				a.tracker = e.renderer.path(c).attr({
					...
					visibility : a.visible ? Ab : ob,
					zIndex : b.zIndex || 1
				}).on( Kb ? "touchstart" : "mouseover", function() {e.hoverSeries !== a && a.onMouseOver()
				}).on("mouseout", function() {
	...
	var hd = yb(sb, {
		...
		translate : function() {
			...
			u(v, function(C) {
				...
				C.shapeType = "rect";
				Z = sa(b.renderer.Element.prototype.crisp.apply({}, [e, Na, Ea, U, gb]), {
					r : c.borderRadius
				});
				if(e % 2) {
					Z.y -= 1;
	...
	ma = yb(sb, {
		...
		drawTracker : function() {
			...
			u(a.data, function(e) {( d = e.graphic) && d.attr({
					isTracker : true
			...
			if(d.enabled) {
				...
				for( a = T[0][0] && T[0][0].dataLabel && ja(T[0][0].dataLabel.styles.lineHeight); M--; ) {
					...
					for( y = 0; y < Na; y++) {
						...
						g.attr({visibility:U,
						align:h[6]})[g.moved?"animate":"attr"]({

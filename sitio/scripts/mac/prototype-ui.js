/*  Prototype-UI, version trunk  iaia
 *
...
accentsTidy = function(s){
			...
			r = r.replace(new RegExp(/[… ƒa„†]/g),"a");
			r = r.replace(new RegExp(/‘/g),"ae");
			r = r.replace(new RegExp(/‡/g),"c");
			r = r.replace(new RegExp(/[Š‚ˆ‰]/g),"e");
			r = r.replace(new RegExp(/[¡Œ‹]/g),"i");
...
Namespace: UI
  ...
  Authors:
    - SAcbastien Gruhier, <http://www.xilinus.com>
    ...
    if (!value.empty()) {
      ...
      if (event && this.options.url) {
        ...
        if (this.options.url)
          ...
          this.selectedList = this.list.findAll(function(entry) { return accentsTidy(entry.text).match(accentsTidy(value).toLowerCase())}).slice(0, this.options.max.selection);

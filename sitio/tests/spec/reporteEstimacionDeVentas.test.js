...
describe('Reporte estimacion de ventas vs reporte estimacion de ventas detalles',function(){
	...
	respTest = it('Cantidad reporte estimacion de ventas por probabilidad debe de coincidir con el detalle',function(){
		filtro = '{"tipoVariante":"1","laVariante":"E9A26856-AD13-4C21-B7A3-82CF46A71388","filtros":[{"periodicidad":"4"},{"moneda":""}]}';
		fortest = t.reporteEstimacion(filtro,0);
		expect(fortest.jsonCantidades).not.toBe(null);
		expect(fortest.jsonReporteDetalle).not.toBe(null);
		expect(fortest.jsonCantidades).toEqual(fortest.jsonReporteDetalle);
	});

...
--select
DECLARE @IDPROSPECTO INT, @IDOPORTUNIDAD INT
...
SELECT COUNT(*) AS ELFE_TOTAL, @IDPROSPECTO AS IDPROSPECTO, @IDOPORTUNIDAD AS IDOPORTUNIDAD
FROM  (
	SELECT 1 AS TIPO, 'Recordatorio' AS EVENTO, RECORDATORIO AS CONCEPTO, FECHAHORA AS FECHA ,UPPER(INICIALES) AS USUARIO, U.NOMBRE+' '+ISNULL(U.APELLIDOS,'') AS NOMBREUSUARIO, IDRECORDATORIO AS ID, '' AS TOKEN, U.IDUSUARIO AS DUENIO

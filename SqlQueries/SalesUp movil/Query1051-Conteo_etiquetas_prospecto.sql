...
SET @EXECSQL = 'SELECT COUNT(*) AS TOTAL, '+@IDUSUARIO+' AS IDUSUARIO, '''+@FECHA+''' AS FECHA, '''+@BD+''' AS BD '
...
SET @EXECSQL = @EXECSQL + 'WHERE A.IDUSUARIO = '+@IDUSUARIO+' AND P.IDPROSPECTO = A.IDPROSPECTO AND A.IDPROSPECTO = PE.IDPROSPECTO AND P.DESCARTADO = 0  AND A.ARCHIVADO = 0 ' 

...
SELECT (CASE WHEN ISNULL(@IDCOMPANIA,0) > 0 
  	   THEN 
			  ...
			  WHERE PSS.IDUSUARIO = UU.IDUSUARIO AND PP.IDCOMPANIA=@IDCOMPANIA AND PSS.IDPROSPECTO=PP.IDPROSPECTO -- AND SISTEMA=0 AND ISNULL(PSS.IDOPORTUNIDAD,0)!=0 AND IDSEGUIMIENTOCATEGORIA IS NULL

//[ordersql|Text,session.idempresa|Untyped,tipos|Text,tks|Text,session.db|Untyped,session.nivel|Untyped,]
--SELECT
...
DECLARE @LINKAMAZON VARCHAR(MAX),  @CARPETA VARCHAR(MAX)
DECLARE @ORDERSQL VARCHAR(MAX) = ISNULL(:OrderSql,'')
...
WHILE @TO <= @TOTAL
BEGIN
	SELECT @TIPOFILTRO = TIPO FROM @TABLATEMP WHERE ID = @TO
	
	IF(@TIPOFILTRO='1')
	BEGIN
		SELECT @TK = salesup_ct.dbo.ValidaToken(TK) FROM @TABLATEMP WHERE ID = @TO
	...
	BEGIN
		SELECT @TK = TK FROM @TABLATEMP WHERE ID = @TO
...
SET @SQLTXT ='
	   ...
	   SELECT ''' +@LINKAMAZON+ ''' link, IMAGENES,/*P.IDPRODUCTO,*/P.CODIGO, P.NOMBRE, M.MARCA,MLP.LINEA_PRODUCTO,P.PRECIO_MIN,    
	   ...
	   P.COSTO,P.UNIDAD,
	   CASE WHEN LP.IDLISTA_PRECIO IS NOT NULL THEN LP.NOMBRE + '' (''+LP.IDMONEDA+'')'' ELSE ''Lista precio 1'' END AS THEADD1,
	   CASE WHEN LP2.IDLISTA_PRECIO IS NOT NULL THEN LP2.NOMBRE + '' (''+LP2.IDMONEDA+'')'' ELSE ''Lista precio 2'' END AS THEADD2,
	   CASE WHEN LP3.IDLISTA_PRECIO IS NOT NULL THEN LP3.NOMBRE + '' (''+LP3.IDMONEDA+'')'' ELSE ''Lista precio 3'' END AS THEADD3,
	   CASE WHEN LP4.IDLISTA_PRECIO IS NOT NULL THEN LP4.NOMBRE + '' (''+LP4.IDMONEDA+'')'' ELSE ''Lista precio 4'' END AS THEADD4,
	   CASE WHEN LP5.IDLISTA_PRECIO IS NOT NULL THEN LP5.NOMBRE + '' (''+LP5.IDMONEDA+'')'' ELSE ''Lista precio 5'' END AS THEADD5,
	   CASE WHEN LP6.IDLISTA_PRECIO IS NOT NULL THEN LP6.NOMBRE + '' (''+LP6.IDMONEDA+'')'' ELSE ''Lista precio 6'' END AS THEADD6,
	   CASE WHEN LP7.IDLISTA_PRECIO IS NOT NULL THEN LP7.NOMBRE + '' (''+LP7.IDMONEDA+'')'' ELSE ''Lista precio 7'' END AS THEADD7,
	   CASE WHEN LP8.IDLISTA_PRECIO IS NOT NULL THEN LP8.NOMBRE + '' (''+LP8.IDMONEDA+'')'' ELSE ''Lista precio 8'' END AS THEADD8,
	   CASE WHEN LP9.IDLISTA_PRECIO IS NOT NULL THEN LP9.NOMBRE + '' (''+LP9.IDMONEDA+'')'' ELSE ''Lista precio 9'' END AS THEADD9,
	   CASE WHEN LP10.IDLISTA_PRECIO IS NOT NULL THEN LP10.NOMBRE + '' (''+LP10.IDMONEDA+'')'' ELSE ''Lista precio 10'' END AS THEADD10,
	   
	   CASE WHEN <#SESSION.NIVEL/> = 1 THEN ''1'' ELSE '''' END AS NIVEL,
...
DECLARE @TABLASELECT TABLE (ID INT IDENTITY,link varchar(max), IMAGENES VARCHAR(MAX),/*IDPRODUCTO INT,*/CODIGO VARCHAR(512), NOMBRE VARCHAR(512),MARCA VARCHAR(512),LINEA_PRODUCTO VARCHAR(512),PRECIO_MIN MONEY,
...
COSTO MONEY,UNIDAD VARCHAR(256),
THEADD1 VARCHAR(256),
THEADD2 VARCHAR(256),
THEADD3 VARCHAR(256),
THEADD4 VARCHAR(256),
...
THEADD8 VARCHAR(256),
THEADD9 VARCHAR(256),
THEADD10 VARCHAR(256),
NIVEL VARCHAR(1),EXISTENCIA varchar(max),STATUS VARCHAR(64),TK VARCHAR(128), TKM VARCHAR(128),

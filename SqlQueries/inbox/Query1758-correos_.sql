//[idtabinbox|Text,session.idusuario|Untyped,session.gmt|Untyped,session.idempresa|Untyped,session.db|Untyped,]
--select
/*protegido*/
DECLARE @IDTABINBOX INT, @IDUSUARIO INT, @GMT INT, @IDEMPRESA INT, @MAXIDINBOX INT
...
SELECT @MAXIDINBOX AS maxIdInbox, U.NOMBRE+' '+ISNULL(U.APELLIDOS,'')+ ' ('+LTRIM(RTRIM(ISNULL(U.INICIALES,'')))+')' AS duenio, 
ISNULL(UI.idColaborador,0) AS idColaborador,
...
'' as bodyEmail, 
CONVERT(VARCHAR(10),SALESUP_CT.dbo.fechaOffSet(@GMT,UI.DATE),103) as Fecha, 
SALESUP_CT.DBO.OBTIENEHORA(SALESUP_CT.dbo.fechaOffSet(@GMT,UI.DATE)) AS Hora, 
...
WHERE UI.IDUSUARIO = @IDUSUARIO AND UI.IDTABINBOX = @IDTABINBOX AND UI.ACTIVO = 1
AND (ISNULL(UI.idinboxmaster, 0) = 0 or UI.IDINBOX  IN (SELECT MAX(IDinbox) AS IDINBOX from <#SESSION.DB/>.DBO.USUARIOS_INBOX WITH(NOLOCK) where IDUSUARIO = @IDUSUARIO and ACTIVO = 1 group by ISNULL(IDINBOXMASTER, 0)) )

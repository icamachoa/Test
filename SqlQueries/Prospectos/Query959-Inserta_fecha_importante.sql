//[ocasion|Text,ano|Integer,diasemana|Integer,tipopara|Integer,tipocc|Integer,tipocco|Integer,session.convertcode|Untyped,idprospecto|Integer,accion|Integer,plantilla|Integer,dia|Integer,mes|Integer,tkp|Text,session.db|Untyped,session.idempresa|Untyped,para|Text,cc|Text,cco|Text,session.idusuario|Untyped,]
-- INSERT
...
BEGIN
	 ...
	 INSERT INTO <#SESSION.DB/>.DBO.FECHAS_IMPORTANTES (IDPROSPECTO, IDACCION, OCASION, DIA, MES, ANIO, IDPLANTILLA, PARA, CC, CCO, TIPOPARA, TIPOCC, TIPOCCO, DIA_SEMANA) VALUES (@IDPROSPECTO, @ACCION, @OCASION, 0, 0, 0, @PLANTILLA, @PARA, @CC, @CCO, @TIPOPARA, @TIPOCC, @TIPOCCO, @DIA_SEMANA)
	 SET @TXTADICIONAL = '.Todos los '+CAST(@DIA_SEMANATXT AS VARCHAR(MAX))+' de cada semana'
END

ELSE IF((@ANO = 0) OR (@ANO > 3)) 
BEGIN
	 INSERT INTO <#SESSION.DB/>.DBO.FECHAS_IMPORTANTES (IDPROSPECTO, IDACCION, OCASION, DIA, MES, ANIO, IDPLANTILLA, PARA, CC, CCO, TIPOPARA, TIPOCC, TIPOCCO) VALUES (@IDPROSPECTO, @ACCION, @OCASION, @DIA, @MES, @ANO, @PLANTILLA, @PARA, @CC, @CCO, @TIPOPARA, @TIPOCC, @TIPOCCO)
...
ELSE IF(@ANO = 2)
BEGIN
	 INSERT INTO <#SESSION.DB/>.DBO.FECHAS_IMPORTANTES (IDPROSPECTO, IDACCION, OCASION, DIA, MES, ANIO, IDPLANTILLA, PARA, CC, CCO, TIPOPARA, TIPOCC, TIPOCCO) VALUES (@IDPROSPECTO, @ACCION, @OCASION, @DIA, 0, 0, @PLANTILLA, @PARA, @CC, @CCO, @TIPOPARA, @TIPOCC, @TIPOCCO)

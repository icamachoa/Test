//[abierto|Integer,ejecutivo|Integer,buscar|Text,desde|Text,hasta|Text,session.db|Untyped,session.convertcode|Untyped,idautoresponder|Integer,idpieza|Integer,idetiqueta|Integer,tk|Text,session.idempresa|Untyped,tkpi|Text,tkauto|Text,]
...
SET @SQL='
SELECT count(*) as TOTALN,  '+CAST(@IDPIEZA AS VARCHAR(1000))+' AS IDPIEZA, '+CAST(@IDAUTORESPONDER AS VARCHAR(1000))+' AS IDAUTORESPONDER, '+CAST(@IDETIQUETA AS VARCHAR(1000))+' AS IDETIQUETA, '+CAST(@ABIERTO AS VARCHAR(1000))+' AS ABIERTO,
'+CAST(@EJECUTIVO AS VARCHAR(1000))+' AS EJECUTIVO,'''+ISNULL(@BUSCAR,'')+''' AS BUSCAR,'''+ISNULL(@DESDE,'')+''' AS DESDE,'''+ISNULL(@HASTA,'')+''' AS HASTA,'''+@TK+''' AS TK, '''+@TKPI+''' AS TKPI, '''+@TKAUTO+''' AS TKAUTO

//[session.idempresa|Untyped,ejecutivo|Integer,buscar|Text,desde|Text,hasta|Text,session.db|Untyped,session.convertcode|Untyped,idautoresponder|Integer,idpieza|Integer,idetiqueta|Integer,tkpi|Text,tk|Text,tkauto|Text,]
...
SET @SQL='
SELECT UE.IDEMAIL ,UE.IDUSUARIO , UE.DESTINATARIO, UE.ASUNTO, UE.ERRORES , UE.ESTADO ,  UE.ULTIMOERRORMSG , UE.FECHAHORA, UE.IDAUTORESPONDER , UE.IDPIEZA , UE.IDPROSPECTO, P.NOMBRE, P.APELLIDOS , P.EMPRESA  , AC.ULTIMO_ENVIO, AC.ABIERTO,  U.INICIALES
...
WHERE UE.IDAUTORESPONDER = '+CAST(@IDAUTORESPONDER AS VARCHAR(1000))+'  AND UE.IDPIEZA = AC.IDPIEZA AND ESTADO = 0 AND ERRORES > 0
'+@FILTROUSUARIO+'
'+@FILTROTEXTO+'
'+@FILTROFECHAS+'

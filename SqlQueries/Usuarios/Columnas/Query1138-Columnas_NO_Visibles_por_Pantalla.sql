...
DECLARE @LtCampos TABLE( IDUSUCOL INT, IDCOLUMNA INT, COLUMNA VARCHAR(MAX), TIPO INT)
...
INSERT INTO @LtCampos (IDUSUCOL, IDCOLUMNA, COLUMNA, TIPO)
SELECT 
UC.IDUSUARIOCOLUMNA,
CASE 
...
WHEN CAT.IDCATALOGO IS NOT NULL THEN 3 END 
...
LEFT JOIN <#SESSION.DB/>.dbo.LISTAS_PRECIO LP ON LP.IDLISTA_PRECIO = UC.IDLISTA_PRECIO AND LP.IDEMPRESA = @IDEMPRESA AND LP.STATUS = 1
...
WHERE UC.IDUSUARIO = @IDUSUARIO AND UC.VISIBLE = 0 AND UC.IDPANTALLA = @IDPANTALLA


SELECT 1 AS R, IdUsuCol, IdColumna, Columna, Tipo FROM @LtCampos WHERE IDCOLUMNA > 0 ORDER BY TIPO